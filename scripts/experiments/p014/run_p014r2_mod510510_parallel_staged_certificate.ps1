[CmdletBinding()]
param([switch]$ConfirmP014R2)

$ErrorActionPreference = 'Stop'
Set-StrictMode -Version Latest
if (-not $ConfirmP014R2) {
    throw 'P014-R2 modulus-510510 parallel staged experiment requires -ConfirmP014R2.'
}
...
$RunId = ([DateTime]::UtcNow.ToString('yyyyMMddTHHmmssZ')) + '_p014r2_mod510510_parallel_staged_certificate'
...
$AnalysisProgressPath = Join-Path $LogDirectory "$RunId.p014r2-analysis.progress.jsonl"
$VerificationProgressPath = Join-Path $LogDirectory "$RunId.p014r2-serial-verification.progress.jsonl"

foreach ($Required in @(
    $Python, $Helper, $AnalysisSource, $ParallelSource, $CliSource, $ProgressSource,
...
try {
    ...
    Write-RunLine '[RUN] experiment=P014R2_MOD510510_PARALLEL_STAGED_CERTIFICATE'
    ...
    Write-RunLine '[PASS] P014-R2 parallel modulus-510510 staged certificate experiment completed.'
