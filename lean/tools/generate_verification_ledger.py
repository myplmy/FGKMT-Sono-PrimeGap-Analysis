...
NOTES_BY_ID = {
    ...
    "81.1": "FGKMT Theorem 3의 sequential partially-sifted filtration 정의; source 구조 전사",
    "81.2": "outer-good과 conditional inner-good의 교집합인 final sieve-good 사건 정의",
    "81.3": "Theory 79의 parameterized final sieve-good mass 재사용; probability measure proof는 미형식화",
    "81.4": "Theory 55 floor의 actual 사건영역을 교정하고 positive terminal algebra를 커널 검증",
    "81.5": "final sieve-good 사건에서만 나누는 normalized squared badness 정의",
    "81.6": "corrected same-law normalized moment 정의",
    "81.7": "Markov probability lower는 premise; strict gate 이후 positive terminal을 커널 검증",
    "81.8": "corrected final sieve-good strict moment gate의 terminal을 조건부 커널 검증",
    "81.9": "final sieve-good 사건에 제한된 raw analytic second-moment input; source theorem OPEN",
    "81.10": "같은 사건의 positive survivor floor를 쓰는 raw-to-normalized 단조성을 커널 검증",
    "81.11": "final residue와 CRT shift의 congruence convention 정의",
    "81.12": "CRT congruence와 한 prime 생존조건의 elementary equivalence; Lean 순차 형식화 대기",
    "81.13": "Theory 55 sifted set과 Theory 80 gcd-survivor set의 exact identification; Lean 순차 형식화 대기",
    "81.14": "한 prime coordinate만 바뀌는 squarefree CRT shift premise 정의",
    "81.15": "두 residue class containment의 초등 finite proof와 Python 전수검산; full Finset proof 대기",
    "81.16": "symmetric-difference bound에서 absolute count bound로 가는 terminal을 커널 검증",
    "81.17": "shifted Dirichlet-character coefficient 정의",
    "81.18": "P=65 finite character 값은 exact Python 검산; 합의 integer arithmetic만 Lean 형식화",
    "81.19": "character 변화 3이 membership bound 2보다 큼의 integer terminal을 커널 검증",
    "81.20": "추가 구조 없는 두 length-N character 합의 trivial bound; Lean 순차 형식화 대기",
    "81.21": "weighted L1 triangle bound; actual PAP-sized analytic weight는 OPEN",
    "81.22": "actual FMT filtration의 Doob increment 정의",
    "81.23": "Freedman형 bounded increment·conditional variance 요구식; actual input OPEN",
    "81.24": "CRT shift 전후 reduced-residue count-difference 함수 정의",
    "81.25": "finite character orthogonality/Parseval source identity; Lean 미형식화",
    "81.26": "weighted increment의 Cauchy--Schwarz 경로; actual complex vector 형식화 대기",
    "83.1": "prescribed primorial power-regime normalization 정의",
    "83.2": "Theory 82 strict character-energy gate 재사용; analytic V input은 OPEN",
    "83.3": "Montgomery--Vaughan 직접 certificate RHS 정의",
    "83.4": "source upper/lower premise 이후 gate가 certificate RHS 아래라는 종단 합성만 조건부 커널 검증",
    "83.5": "upper RHS가 gate 위이면 strict 결론이 논리적으로 나오지 않는 countermodel을 부분형식화",
    "83.6": "power exponent delta=1/d 정의와 endpoint 전사",
    "83.8": "Fiorilli--Martin Proposition 2.2 source implication; analytic proof 미형식화",
    "83.9": "Theory 77에서 이미 검사한 rational endpoint 재사용; display 전체는 부분형식화",
    "83.10": "Montgomery--Vaughan Theorem 1 source statement; 원문·페이지·hash 고정",
    "83.11": "Farey spacing을 사용한 additive specialization; finite complex proof 미형식화",
    "83.12": "primitive Gauss sum·Bessel multiplicative large-sieve source 합성; Lean 미형식화",
    "83.13": "Lambda-square sum과 L 표기 정의",
    "83.14": "Dusart Theorem 5.2 source theorem; Lean 독립증명 없음",
    "83.15": "Dusart endpoint substitution의 실수 analytic 합성; Python 고정밀 검산",
    "83.15a": "L=8 Dusart coefficient의 rational lower certificate를 커널 검증; 실수로그 monotonicity는 문서 proof",
    "83.16": "current L range에서 quarter lower; source log inequalities Lean 미형식화",
    "83.17": "prime half-interval로 Lambda-square lower를 얻는 문서 proof; Lean 미형식화",
    "83.18": "large-sieve published RHS의 source-derived lower envelope",
    "83.19": "L>=21t와 log2<t 이후 5t strict margin을 조건부 커널 검증",
    "83.20": "q개 atom의 finite pigeonhole lower; Lean 순차 형식화 대기",
    "83.21": "best-case probability·survivor factor 범위 전사",
    "83.22": "각 nonnegative factor 상계에서 best-entropy gate envelope로 가는 곱셈 단조성을 조건부 커널 검증",
    "83.23": "Rosser--Schoenfeld 일반식 source theorem; Lean 독립증명 없음",
    "83.24": "exception coefficient를 전 q에 확대한 안전 envelope; source theorem 미형식화",
    "83.25": "strict 비교용 scalar 함수 정의",
    "83.26": "미분식·양수성 문서 proof와 Python 진단; transcendental source 전부는 Lean 미형식화",
    "83.27": "elementary exp/log endpoint interval 문서 proof; Lean 순차 형식화 대기",
    "83.28": "120-dps endpoint 교차검산; directed interval certificate는 아님",
    "83.29": "V=gate exact countermodel로 raw upper certificate의 strict-gate 논리 불충분성을 커널 검증",
