...
## 현재 상태: P002–P012-B 단계별 PASS / P012-B 사용자 시각 QA 대기 / P010A G4 상한 개선 PASS / P010B verifier toy PASS·가속 BLOCKED
...
- P012-B figure 자동 QA PASS; 사용자 시각 QA 대기
...
다음 권장 행동은 사용자가 P012-B expected/residual PNG 두 장의 축·선·막대·undefined-z
marker·범례·잘림을 시각검사하는 것이다. 완료 runner는
`test_done/run_p012_stratified_null_holdout-20260827T121734Z-done.ps1`로 이관했으므로 재실행하지
않는다. 시각 QA 뒤 recurrence 축은 새 P013에서 검정력·식별가능성을 먼저 설계한다.
계산수론 축의 다음 구현 우선순위는 P010B compressed absolute coverage schema와 verifier이며,
exhaustive truth 없이 누락 없는 mapping과 PARI witness adapter가 준비되기 전에는 actual
acceleration runner를 만들지 않는다. 현재 과학적으로 타당하고 code-ready인 3–12시간 runner는
없다. 상세 감사는 `docs/method/20260827_3to12h_compute_readiness_audit.md`다.
