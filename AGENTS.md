...
closed Theorem 6 합에서 외부 \((T,2T]\) prime set으로 돌아갈 때의 \(w(T)\), P92의
weighted error multiplier와 common cutoff가 열려 있어 broad `SIV-08`은
`HARD_BLOCKER`, \(X_{\mathrm{cert}}\)는 `OPEN`이다. 모든 root dependency가
...
다음 권장 행동은 P018-B를 자동 실행하지 않고 H1b-P92a로 진행하는 것이다.
H1c-1b.4e가 actual identity-form Hypothesis 1 입력을 닫았으므로, Maynard Proposition 9.2의
weighted main/error term과 FGKMT closed-to-\((T,2T]\) lower-endpoint weight를 별도로
명시화한다. 이 둘을 닫기 전에는 P92나 broad `SIV-08`을 승격하지 않는다.
그 뒤 P91/L93/P95와 공통 moment error budget을 합성한다.
